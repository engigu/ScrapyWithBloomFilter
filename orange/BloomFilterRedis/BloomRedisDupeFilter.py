...
class BloomRedisDupeFilter(BaseDupeFilter):
    ...
    def __init__(self, key, host, port, db, hash_func_list, blockNum, debug=False):
        self.bloomFilterRedis = BloomFilterRedis(key=key, host=host, port=port, db=db, blockNum=blockNum,
                                                 hash_func_list=hash_func_list, debug=debug)
        self.logdupes = True
        self.debug = debug
    ...
    def from_settings(cls, settings):
        ...
        return cls(key, host, port, db, hash_func_list, block_num, debug)
